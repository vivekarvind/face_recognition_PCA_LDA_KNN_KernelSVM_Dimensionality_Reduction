function [] = Task6_Vivek_PCA_KERNEL_SVM()
    ...
    for i=1:5
        ...
        %PCA
        warning('off','all');
        number_x = size(train_faces, 2);
        mean_face = mean(train_faces, 2);
        centered_data = train_faces - repmat(mean_face, [1, number_x]);
        mat_gram = centered_data'*centered_data;
        [eigen_vectors, eigen_values] = eigs( mat_gram, number_x );
        eigen_vectors = train_faces*eigen_vectors;
        eigen_vectors = orth(eigen_vectors);
        eigen_values = diag(eigen_values);
        partial_sum = cumsum(eigen_values);
        ratio_value = partial_sum/partial_sum(end);
        k_value = find(ratio_value>0.9, 1, 'first');
        e = eigen_vectors(:, 1:k_value);
    ...
    fprintf('Overall Accuracy of PCA + Kernel SVM is: %.2f%%\n', 100 * mean(accuracy));
end
